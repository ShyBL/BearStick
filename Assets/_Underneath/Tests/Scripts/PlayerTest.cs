...
public class PlayerTest : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (_isGrounded)
        {
            Move(MoveStats.GroundAcceleration, MoveStats.GroundDeceleration, TestInputManager.Movement);
            Move(MoveStats.GroundAcceleration, MoveStats.GroundDeceleration, TestInputManager.Movement);

        }
        else
        {
            Move(MoveStats.AirAcceleration, MoveStats.AirDeceleration, TestInputManager.Movement);
        }
    ...
    {
        if (TestInputManager.JumpWasPressed)
        {
            _jumpBufferTimer = MoveStats.JumpBufferTime;
            _jumpReleasedDuringBuffer = false;
        }

        if (TestInputManager.JumpWasReleased)
        {
